...
export class OpenWithRuntime extends AppProcess {
  ...
  async render({ path }: RenderArgs) {
    ...
    const available = await this.userDaemon!.files!.findHandlerToOpenFile(path);

    this.available.set(available);
    this.all.set(await this.userDaemon!.files!.getAllFileHandlers());
  ...
  async go(id = this.selectedId()) {
    ...
    if (this.userDaemon?.files!.fileHandlers?.[id]) return await this.userDaemon?.files!.fileHandlers?.[id]?.handle(this.path());
