...
export class OpenWithRuntime extends AppProcess {
  ...
  async render({ path }: RenderArgs) {
    ...
    const available = await this.userDaemon!.filesystemContext!.findHandlerToOpenFile(path);

    this.available.set(available);
    this.all.set(await this.userDaemon!.filesystemContext!.getAllFileHandlers());
  ...
  async go(id = this.selectedId()) {
    ...
    if (this.userDaemon?.filesystemContext!.fileHandlers?.[id]) return await this.userDaemon?.filesystemContext!.fileHandlers?.[id]?.handle(this.path());
