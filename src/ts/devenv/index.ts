...
export class DevelopmentEnvironment extends BaseService {
  ...
  async connect(port: number): Promise<DevEnvActivationResult> {
    ...
    return new Promise((r) => {
        ...
        MessageBox(
          {
            ...
            image: this.daemon.appRegistrationContext!.getIconCached("ErrorIcon"),
      ...
      this.client.on("open-file", (file: string) => {
        if (this._disposed) return this.disconnect();
        this.daemon.filesystemContext!.openFile(file);
      });
  ...
  async restartTpa() {
    ...
    await this.killTpa();
    await this.daemon.filesystemContext!.openFile("V:/_app.tpa");
  }
