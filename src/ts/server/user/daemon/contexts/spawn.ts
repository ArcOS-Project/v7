...
export class SpawnUserContext extends UserContext {
  ...
  async spawnApp<T>(id: string, parentPid?: number, ...args: any[]) {
    ...
    return await this._spawnApp<T>(id, this.userDaemon.workspacesContext?.getCurrentDesktop(), parentPid, ...args);
  }

  async spawnOverlay<T>(id: string, parentPid?: number, ...args: any[]) {
    if (this._disposed) return;

    return await this._spawnOverlay<T>(id, this.userDaemon.workspacesContext?.getCurrentDesktop(), parentPid, ...args);
  }
  ...
  async _spawnApp<T>(
    ...
    if (this.userDaemon.applicationsContext?.checkDisabled(id, app?.noSafeMode)) return;
  ...
  async spawnThirdParty<T>(app: App, metaPath: string, ...args: any[]): Promise<T | undefined> {
    ...
    if (!this.userDaemon.preferences().security.enableThirdParty) {
    ...
    if (this.userDaemon!.autoLoadComplete) stop = (await this.GlobalLoadIndicator(`Opening ${app.metadata.name}...`)).stop;
  ...
  tpaError_noEnableThirdParty() {
    if (this.userDaemon!.autoLoadComplete)
      MessageBox(
        {
