...
export class AppRenderer extends Process {
  ...
  _renderTitlebar(process: AppProcess) {
    ...
    titleIcon.src = process.userDaemon?.appRegistrationContext!.getAppIconByProcess(process) || process.getIconCached("ComponentIcon");
    ...
    if (BETA && !process.app.data.entrypoint && !process.app.data.workingDirectory && !process.app.data.thirdParty) {
      ...
      feedbackButton.addEventListener("click", () => {
        process.userDaemon?.checksContext!.iHaveFeedback(process);
      });
