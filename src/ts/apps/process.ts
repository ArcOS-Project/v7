...
export class AppProcess extends Process {
  ...
  constructor(pid: number, parentPid: number, app: AppProcessData, ...args: any[]) {
    ...
    this.windowIcon.set(this.userDaemon?.appRegistrationContext!.getAppIconByProcess(this) || this.getIconCached("ComponentIcon"));
          ...
          buttons: [
            {
              caption: "Manage apps",
              action: () => {
                this.userDaemon?.spawnContext?.spawnApp("systemSettings", +this.env.get("shell_pid"), "apps", "apps_manageApps");
              },
            },
          ],
  ...
  async closeIfSecondInstance() {
    ...
    if (instances.length) {
      ...
      if (instances[0].app.desktop) this.userDaemon?.workspacesContext!.switchToDesktopByUuid(instances[0].app.desktop);
    }
  ...
  private async processor(e: KeyboardEvent) {
    ...
    for (const combo of this.acceleratorStore) {
      ...
      if (!this.userDaemon?.elevationContext!._elevating) await combo.action(this, e);
  ...
  async spawnApp<T = AppProcess>(id: string, parentPid?: number | undefined, ...args: any[]) {
    return await this.userDaemon?.spawnContext?.spawnApp<T>(id, parentPid ?? this.parentPid, ...args);
  }

  async spawnOverlayApp<T = AppProcess>(id: string, parentPid?: number | undefined, ...args: any[]) {
    return await this.userDaemon?.spawnContext?.spawnOverlay<T>(id, parentPid ?? this.parentPid, ...args);
  }

  async elevate(id: string) {
    if (!this.elevations[id]) return false;
    return await this.userDaemon!.elevationContext!.manuallyElevate(this.elevations[id]);
  }
  ...
  notImplemented(what?: string) {
    ...
    this.userDaemon?.spawnContext?.spawnOverlay("messageBox", this.pid, {
  ...
  async getIcon(id: string): Promise<string> {
    return this.userDaemon?.appRegistrationContext!.getIcon(id)!;
  }

  getIconCached(id: string): string {
    return this.userDaemon?.appRegistrationContext!.getIconCached(id)!;
  }

  getIconStore(id: string): ReadableStore<string> {
    return this.userDaemon?.appRegistrationContext!.getIconStore(id)!;
  }
}
