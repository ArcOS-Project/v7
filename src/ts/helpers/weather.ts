import type { WeatherInformation, WeatherMeta } from "$types/external/weather";
import { fetchWeatherApi } from "openmeteo";
import { BaseHelper } from ".";

export class WeatherHelper extends BaseHelper {
  static override FriendlyName: string = "Weather info helper";

  public static async getWeather(): Promise<WeatherInformation> {
    ...
    try {
      ...
      const metadata = WeatherHelper.weatherMetadata[weather_code]!;

      return {
        code: weather_code,
        condition: metadata.caption,
        temperature: temperature_2m,
        className: WeatherHelper.weatherClasses[weather_code],
  ...
  public static weatherMetadata: Record<number, WeatherMeta> = {
    0: {
      caption: "Clear sky",
      gradient: {
        start: "#4facfe",
  ...
  public static weatherClasses: Record<number, string> = Object.fromEntries(
    Object.entries(WeatherHelper.weatherMetadata).map(([n, m]) => [n, m.caption.replaceAll(" ", "").toLowerCase()])
  );
}
